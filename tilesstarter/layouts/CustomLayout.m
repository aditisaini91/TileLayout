...
- (BOOL) _traverseOpenTilesForTileWithSize:(CGSize)size andIndexPath:(NSIndexPath*)indexPath{
    ...
    for(int unrestrictedDimension = self.firstOpenSpace.y;; unrestrictedDimension+=smallestTileHeightInRow) {
        int restrictedDimension = self.firstOpenSpace.x;
        
        while (restrictedDimension  <  MAX_WIDTH) {
            
            //if the tile could not be accomodated at this point(there is not enough space left), then there is no point of searching for open points till the end
            if (restrictedDimension + size.width > MAX_WIDTH) {
                self.firstOpenSpace = CGPointMake(0, self.smallestTilePointY);
                break;
            }
            ...
            restrictedDimension = self.firstOpenSpace.x + 1;
        }
    }
